...
int main(void)
{
    ...
    // Using indexing is very simple. 
    // First, you need to create an additional buffer, 
    // which you fill with the right indices. 
    // The code is the same as before, 
    // but now it’s an ELEMENT_ARRAY_BUFFER, not an ARRAY_BUFFER.
    ...
    do{
        // Set a FPS counter to Measure speed
        ...
        // to draw the mesh, simply replace glDrawArrays by this
        // Index buffer
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, elementbuffer);
		...
		// Again : this is already done, but this only works 
        // because we use the same shader.
		//// Bind our texture in Texture Unit 0
		//glActiveTexture(GL_TEXTURE0);
		//glBindTexture(GL_TEXTURE_2D, Texture);
		//// Set our "myTextureSampler" sampler to use Texture Unit 0
